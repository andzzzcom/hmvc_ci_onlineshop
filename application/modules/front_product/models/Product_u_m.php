...
class Product_u_m extends CI_Model {
	...
	public function get_related_produk($data)
	{
		$this->db->select('*');
		...
		$this->db->from('tb_produk p');
		$this->db->where('s.id_sub_kategori', $data["id_sub_kategori"]);
		$this->db->where('p.id_produk !=', $data["id_produk"]);
	...
	public function get_recommended_produk($limit)
	{
		$this->db->select('*');
	...
	public function get_latest_product($limit)
    {
		$this->db->select('*');
		$this->db->from('tb_produk b'); 
		$this->db->order_by('b.id_produk', 'DESC');
