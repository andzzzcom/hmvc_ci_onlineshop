					...
					<form method="post" onsubmit="return validasi();" enctype="multipart/form-data" action="<?php echo base_url();?>profile/shipping_settings_action">
						<div class="col-md-12 text-left">
							...
							<div class="form-group">
								<label>Kurir</label>
								<select id="id_kurir" required class="form-control" name="id_kurir">
								<?php
									foreach($courier_list as $cour)
									{
								?>
									<option class="form-control" <?php if($data_user[0]->kurir_user == $cour->code_kurir) echo"selected"; ?> value="<?php echo $cour->code_kurir; ?>"><?php echo $cour->name_kurir;?></option>
								<?php
									}
								?>
								</select>
